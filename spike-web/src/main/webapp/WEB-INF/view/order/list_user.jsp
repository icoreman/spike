...
<body>
		...
		<c:forEach items="${list}" var="item">
			<tr>
				...
				<td><c:if test="${item.payStatus == 1 }">
						<a href="topay?id=${item.id}">继续支付</a>
					</c:if> <c:if test="${item.payStatus == 2 }">支付完成<input
							type="button" value="发起退款"
							onclick="applyrefund('${item.id}','${item.payType}')">
					</c:if> 
	...
	<script type="text/javascript">
		function applyrefund(orderid, paytype) {
			window.location.href = "applyrefund?orderid=" + orderid
					+ "&&paytype=" + paytype;
		}
	</script>
</body>
